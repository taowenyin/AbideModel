# download_abide_struc_ants_cortical.py.py
#
...
Usage:
    python download_abide_struc_ants_cortical.py.py -d <derivative> -p <pipeline>
...
def collect_and_download(derivative, pipeline, out_dir, less_than, greater_than, site, sex, diagnosis):
    ...
    for pheno_row in pheno_list[1:]:
        ...
        if greater_than < row_age < less_than:
            ...
            s3_path = '/'.join([s3_prefix, 'Outputs', pipeline, derivative, filename])
...
if __name__ == '__main__':
    ...
    for desired_derivative in desired_derivative_list:
        # 构建保存地址
        output_data_dir = os.path.join(save_data_dir, desired_pipeline, desired_derivative)

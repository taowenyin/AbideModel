# download_abide_struc_civet_cortical.py
#
...
Usage:
    python download_abide_struc_civet_cortical.py -d <derivative> -p <pipeline>
...
def collect_and_download(derivative, pipeline, out_dir, less_than, greater_than, site, sex, diagnosis):
    ...
    for pheno_row in pheno_list[1:]:
        ...
        if greater_than < row_age < less_than:
            ...
            s3_path = '/'.join([s3_prefix, 'Outputs', pipeline, 'thickness_' + derivative, filename])
...
if __name__ == '__main__':
    ...
    # 表面的类型，6个参数获取选取不定数量
    # ['cerebral_volume', 'native_rms_rsl_tlink_30mm_asym_hemi',
    # 'native_rms_rsl_tlink_30mm_left', 'native_rms_rsl_tlink_30mm_right']
    desired_derivative_list = ['native_rms_rsl_tlink_30mm_asym_hemi']

    for desired_derivative in desired_derivative_list:
        # 构建保存地址
        output_data_dir = os.path.join(save_data_dir, desired_pipeline, 'cortical', desired_derivative)
